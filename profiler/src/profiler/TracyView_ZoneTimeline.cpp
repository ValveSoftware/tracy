...
void View::DrawCpuTrack( const TimelineContext &ctx, uint64_t coreIndex, const std::vector<TimelineDraw> &draw, const std::vector<ContextSwitchDraw> &ctxDraw, const HwCounterDraw &hwCounterDraw, int &offset, int depth )
{
    ...
    const auto hasCtxSwitch = m_vd.drawContextSwitches;
    ...
    const auto depthLimitIt = m_coreDepthLimit.find( coreIndex );
    const bool croppingActive = ( depthLimitIt != m_coreDepthLimit.end() && depthLimitIt->second <= depth );
    const int croppedDepth = croppingActive ? depthLimitIt->second : depth;
    ...
    if( !draw.empty() && yPos <= yMax && yPos + ostep * croppedDepth >= yMin )
    {
        ...
        DrawZoneList( ctx, draw, offset, coreIndex, croppedDepth, croppingActive ? cropperAdditionalMargin + GetScale() /* Ensure text has a bit of space for text */ : 0.f );
    }
    ...
    {
        ImGui::PopClipRect();
        if( depth > 0 ) DrawThreadCropper( depth, coreIndex, croppperPosX, yPos, ostep, cropperWidth, hasCtxSwitch, m_coreDepthLimit );
    }
}
...
void View::DrawThread( const TimelineContext &ctx, const ThreadData &thread, const std::vector<TimelineDraw> &draw, const std::vector<ContextSwitchDraw> &ctxDraw, const std::vector<SamplesDraw> &samplesDraw, const std::vector<std::unique_ptr<LockDraw>> &lockDraw, const HwCounterDraw &hwCounterDraw, int &offset, int depth, bool _hasCtxSwitches, bool _hasSamples, bool _hasHwCounter )
{
    ...
    {
        ImGui::PopClipRect();
        if( depth > 0 ) DrawThreadCropper( depth, thread.id, croppperPosX, yPos, ostep, cropperWidth, hasCtxSwitch, m_threadDepthLimit );
    }
}
...
void View::DrawZoneList( const TimelineContext& ctx, const std::vector<TimelineDraw>& drawList, int _offset, uint64_t tidOrCoreIndex, int maxDepth, double margin )
{
    auto draw = ImGui::GetWindowDrawList();
    const auto w = ctx.w;
    const auto wpos = ctx.wpos;
    ...
    const auto DrawZoneText = [&]( uint32_t color, const char* zoneName, ImVec2 tsz, double pr0, double pr1, double px0, double px1, double offset, const int64_t rstart, const int64_t rend ){
    ...
    for( auto& v : drawList )
    {
        ...
        case TimelineDrawType::Folded:
        {
            ...
            const ZoneEvent* pEvent = (const ZoneEvent*)v.ev.get();
            if ( (pEvent != nullptr) && (( v.subtype == TimelineDrawSubType::Thread ) || ( v.num <= 1 )) )
            {
            ...
            {
                if( IsMouseClickReleased( 1 ) ) m_setRangePopup = RangeSlim { rstart, rend, true, nullptr };
                if( v.num > 1 )
                {
                    ImGui::BeginTooltip();
                    TextFocused( "Zones too small to display:", RealToString( v.num ) );
                    ImGui::Separator();
                    TextFocused( "Merged time:", TimeToString( rend - rstart ) );
                    ImGui::Separator();
                    TextFocused( "Start time:", TimeToStringExact( rstart ) );
                    TextFocused( "End time:", TimeToStringExact( rend ) );
                    ImGui::EndTooltip();

                    if( IsMouseClicked( 2 ) && rend - rstart > 0 )
                    {
                        ZoomToRange( rstart, rend );
                    }
                }
                else
                {
                    assert( pEvent );
                    const ZoneEvent& ev = *pEvent;

                    ZoneTooltipClamped( ev, rstart, rend );
        ...
        case TimelineDrawType::Zone:
        {
            ...
            DrawZoneText( 0xFFFFFFFF, zoneName, tsz, pr0, pr1, px0, px1, offset, rstart, rend );

            if( hover && ImGui::IsMouseHoveringRect( wpos + ImVec2( px0, offset ), wpos + ImVec2( px1, offset + tsz.y + 1 ) ) )
            {
                ZoneTooltipClamped( ev, rstart, rend );
                if( IsMouseClickReleased( 1 ) ) m_setRangePopup = RangeSlim { rstart, rend, true, &ev };
        ...
        {
            assert( v.subtype == TimelineDrawSubType::Thread );
            ...
            if( hover && ImGui::IsMouseHoveringRect( wpos + ImVec2( std::max( px0, -10.0 ), offset ), wpos + ImVec2( std::min( std::max( px1, px0+MinVisSize ), double( w + 10 ) ), offset + ty + 1 ) ) )
            {
                if( IsMouseClickReleased( 1 ) ) m_setRangePopup = RangeSlim { ev.start.Val(), rend , true, nullptr };
                ImGui::BeginTooltip();
        ...
        case TimelineDrawType::Ghost:
        {
            ...
            if( !frame )
            {
                ...
                DrawZoneText( txtColor, symName, tsz, pr0, pr1, px0, px1, offset, rstart, rend );

                if( hover && ImGui::IsMouseHoveringRect( wpos + ImVec2( px0, offset ), wpos + ImVec2( px1, offset + tsz.y + 1 ) ) )
                {
                    if( IsMouseClickReleased( 1 ) ) m_setRangePopup = RangeSlim { ev.start.Val(), ev.end.Val() , true, nullptr };
                    ImGui::BeginTooltip();
...
void View::DrawThreadCropper( const int depth, const uint64_t tid, const float xPos, const float yPos, const float ostep, const float cropperWidth, const bool hasCtxSwitches, unordered_flat_map<uint64_t, int> &depthLimitLut )
{
    const ImVec2 mousePos = ImGui::GetMousePos();
    const bool clicked = ImGui::IsMouseClicked( 0 );
    auto draw = ImGui::GetWindowDrawList();
    bool isCropped = ( depthLimitLut.find( tid ) != depthLimitLut.end() );
    const int depthLimit = isCropped ? depthLimitLut[tid] : depth;
    // If user changes settings to hide Ctx Switches, he would be unable to remove the limit, so set the value to its minimum
    if( !hasCtxSwitches && isCropped && depthLimit == 0 ) depthLimitLut[tid] = 1;
    ...
    for( ; lane < depthLimit; lane++ )
    {
        ...
        if( dx * dx + dy * dy <= hradius * hradius )
        {
            ...
            if( clicked )
            {
                ...
                {
                    depthLimitLut.erase( tid );
                }
                else
                {
                    depthLimitLut[tid] = newDepthLimit;
                }
            }
        }
